...
func GetTagList()([]map[string]interface{},error) {
	tag,err := bootstrap.GetDb().Table("tag").Get()
	if err != nil{
		return nil,err
	}
...
func InsertTag(function func()map[string]interface{})(bool,error)  {
	insertData := function()
	result,err := bootstrap.GetDb().Table("tag").Data(insertData).Insert()
	if err != nil {
		return false,err
	}
	if result == 0 {
...
func EditStatus(status string,id string) (bool,error) {
	...
	result,err := bootstrap.GetDb().Table("tag").Data(updateData).Where(map[string]interface{}{"id":id}).Update()
	if err != nil{
		return  false,err
	}
	if result > 0 {
...
func EditTag(callback func()map[string]interface{},id string)(bool,error)  {
	updateData  := callback()
	result,err := bootstrap.GetDb().Table("tag").Where(map[string]interface{}{"id":id}).Data(updateData).Update()
	if err != nil{
		return  false,err
	}
...
func DelTag(id string)(bool,error)  {
	result,err := bootstrap.GetDb().Table("tag").Where(map[string]interface{}{"id":id}).Delete()
	if err != nil{
		return false,err
	}
	if result > 0{
